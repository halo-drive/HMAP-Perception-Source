...
#include "../../plugins/livox_hap_plugin/LivoxHapPlugin.hpp"
...
private:
    ...
    dwSensorPluginSensorHandle_t m_lidarPluginHandle = nullptr;
...
public:
    ...
    void computeSpin()
    {
        ...
        static const int MAX_PACKETS_PER_FRAME = 50; // Safety limit
        static const int FRAME_TIMEOUT_MS = 100;     // Timeout for packet reading
        
        dwStatus status = DW_NOT_AVAILABLE;
        
        // Read packets until we have enough points or max packets
        while (packetCount < MAX_PACKETS_PER_FRAME) {
            status = dwSensorLidar_readPacket(&nextPacket, FRAME_TIMEOUT_MS * 1000, m_lidarSensor);
            ...
            if (status == DW_SUCCESS) {
                ...
                std::cout << "Packet: points=" << nextPacket->nPoints 
                        << ", total accumulated=" << m_accumulatedPoints.size()
        ...
        // If we've collected MAX_PACKETS_PER_FRAME but don't have enough points yet,
        // display what we have
        if (!m_accumulatedPoints.empty()) {
            std::cout << "Max packets reached: " << m_accumulatedPoints.size() << " accumulated points" << std::endl;
            ...
            std::cout << "Max packets reached but no points accumulated" << std::endl;
        }
    }
