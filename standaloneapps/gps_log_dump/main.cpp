...
static std::ofstream gDumpFile;
static bool gDumpToFile   = false;
...
void dumpHeader()
{
    std::ostream& out = gDumpToFile ? gDumpFile : std::cout;
    out << "dump_dwGPSFrame,"
              << "timestamp_us,"
              << "latitude,"
              << "longitude,"
...
int main(int argc, const char** argv)
{
    ...
    for (int32_t i = 0; i < numSensors; i++)
    {
        ...
        // If dump is enabled, extract file path from params and open it for writing (only for first sensor)
        if (enableDumpAll && i == 0)
        {
            std::string filePath;
            size_t filePos = parameterString.find("file=");
            if (filePos != std::string::npos)
            {
                size_t start = filePos + 5; // "file=" is 5 characters
                size_t end = parameterString.find(",", start);
                if (end == std::string::npos)
                    end = parameterString.length();
                filePath = parameterString.substr(start, end - start);
                
                gDumpFile.open(filePath, std::ios::out);
                if (gDumpFile.is_open())
                {
                    gDumpToFile = true;
                    std::cout << "Writing CSV dump to: " << filePath << std::endl;
                }
                else
                {
                    std::cerr << "WARNING: Failed to open file '" << filePath 
                              << "' for CSV dump, writing to stdout instead" << std::endl;
                    gDumpToFile = false;
                }
            }
            else
            {
                std::cout << "CSV dump enabled, but no file= parameter found. Writing to stdout." << std::endl;
                gDumpToFile = false;
            }
        }
