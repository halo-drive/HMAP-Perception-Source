...
// without an express license agreement from NVIDIA 600CORPORATION or
...
private:
    ...
    void initializeStereoCamera()
    {
        if (m_inputTypeCamera)
        {
            ...
            size_t emptyP[1] = {0};
            CHECK_DW_ERROR(dwImage_createAndBindBuffer(&m_stereoFrames[0], m_imageProperties, empty, emptyP, 1, m_context));
            CHECK_DW_ERROR(dwImage_createAndBindBuffer(&m_stereoFrames[1], m_imageProperties, empty, emptyP, 1, m_context));
        }
        else
        {
    ...
    std::unique_ptr<SimpleCamera> initFromCamera()
    {
        ...
        std::string parameters = "camera-n fame=F008A120RM0AV2,interface=csi-ab,CPHY-mode=1,link=3,output-format=processed,async-record=1,file-buffer-size=16777216";
