...
private:
    ...
    // Live GMSL sensors (optional when --live is enabled)
    dwSensorHandle_t m_leftSensor                              = DW_NULL_HANDLE;
    dwSensorHandle_t m_rightSensor                             = DW_NULL_HANDLE;
    dwCameraFrameHandle_t m_leftFrame                          = DW_NULL_HANDLE;
    dwCameraFrameHandle_t m_rightFrame                         = DW_NULL_HANDLE;
    ...
    bool m_inputTypeCamera = false; // legacy USB path (unused in GMSL live)
    bool m_liveGMSL        = false; // new flag to enable GMSL live via rig
...
public:
        ...
        : DriveWorksSample(args)
    {
        ...
        // enable live GMSL path if requested
        const auto liveOpt = args.get("live");
        m_liveGMSL = (!liveOpt.empty() && (liveOpt == "1" || liveOpt == "true"));
    }
    ...
    bool onInitialize() override
    {
        ...
        // Load rig before setting up cameras so GMSL live can use it
    ...
    {
        // Return outstanding frames first (GMSL live)
        if (m_liveGMSL)
        {
            if (m_leftFrame)
            {
                dwSensorCamera_returnFrame(&m_leftFrame);
                m_leftFrame = DW_NULL_HANDLE;
            }
            if (m_rightFrame)
            {
                dwSensorCamera_returnFrame(&m_rightFrame);
                m_rightFrame = DW_NULL_HANDLE;
            }
        }
        ...
        // Stop and release sensors for GMSL live
        if (m_liveGMSL)
        {
            if (m_leftSensor != DW_NULL_HANDLE)
            {
                dwSAL_releaseSensor(m_leftSensor);
                m_leftSensor = DW_NULL_HANDLE;
            }
            if (m_rightSensor != DW_NULL_HANDLE)
            {
                dwSAL_releaseSensor(m_rightSensor);
                m_rightSensor = DW_NULL_HANDLE;
            }
        }
...
private:
    ...
    void initializeStereoCamera()
    {
        if (m_liveGMSL)
        {
            // GMSL live path using rig sensors (left/right by name)
            // Sensors are created after rig is initialized in initializeCalibratedCameras()
            // Create SAL sensors for left and right using rig-provided protocol/params
            const char* leftProtocol = nullptr;
            const char* leftParams   = nullptr;
            CHECK_DW_ERROR(dwRig_getSensorProtocol(&leftProtocol, m_cameraLeftSensorIdx, m_rigConfiguration));
            CHECK_DW_ERROR(dwRig_getSensorParameterUpdatedPath(&leftParams, m_cameraLeftSensorIdx, m_rigConfiguration));

            const char* rightProtocol = nullptr;
            const char* rightParams   = nullptr;
            CHECK_DW_ERROR(dwRig_getSensorProtocol(&rightProtocol, m_cameraRightSensorIdx, m_rigConfiguration));
            CHECK_DW_ERROR(dwRig_getSensorParameterUpdatedPath(&rightParams, m_cameraRightSensorIdx, m_rigConfiguration));

            dwSensorParams leftSP{};  leftSP.protocol  = leftProtocol;  leftSP.parameters  = leftParams;
            dwSensorParams rightSP{}; rightSP.protocol = rightProtocol; rightSP.parameters = rightParams;

            CHECK_DW_ERROR(dwSAL_createSensor(&m_leftSensor, leftSP, m_sal));
            CHECK_DW_ERROR(dwSAL_createSensor(&m_rightSensor, rightSP, m_sal));

            // Start SAL then sensors
            CHECK_DW_ERROR(dwSAL_start(m_sal));
            CHECK_DW_ERROR(dwSensor_start(m_leftSensor));
            CHECK_DW_ERROR(dwSensor_start(m_rightSensor));

            // Query one camera's image properties to set pipeline sizes
            dwImageProperties imgProps{};
            CHECK_DW_ERROR(dwSensorCamera_getImageProperties(&imgProps, DW_CAMERA_OUTPUT_CUDA_RGBA_UINT8, m_leftSensor));
            m_imageProperties.type   = DW_IMAGE_CUDA;
            m_imageProperties.format = DW_IMAGE_FORMAT_RGBA_UINT8;
            m_imageProperties.width  = imgProps.width;
            m_imageProperties.height = imgProps.height;

            m_inputText = "Input " + std::to_string(m_imageProperties.width) + "x" + std::to_string(m_imageProperties.height);
        }
        else if (m_inputTypeCamera)
        {
    ...
    bool readStereoImages(dwImageHandle_t gStereoImages[DW_STEREO_SIDE_BOTH])
    {
        ...
        if (m_liveGMSL)
        {
            // Read frames from left/right GMSL cameras
            m_leftFrame  = DW_NULL_HANDLE;
            m_rightFrame = DW_NULL_HANDLE;

            dwStatus stL = dwSensorCamera_readFrame(&m_leftFrame, 333333, m_leftSensor);
            dwStatus stR = dwSensorCamera_readFrame(&m_rightFrame, 333333, m_rightSensor);

            if (stL != DW_SUCCESS || stR != DW_SUCCESS)
            {
                // Handle end of stream or timeouts by resetting sensors
                if (stL == DW_END_OF_STREAM) CHECK_DW_ERROR(dwSensor_reset(m_leftSensor));
                if (stR == DW_END_OF_STREAM) CHECK_DW_ERROR(dwSensor_reset(m_rightSensor));
                return false;
            }

            // Get CUDA RGBA images from frames
            dwImageHandle_t leftRGBA  = DW_NULL_HANDLE;
            dwImageHandle_t rightRGBA = DW_NULL_HANDLE;
            CHECK_DW_ERROR(dwSensorCamera_getImage(&leftRGBA, DW_CAMERA_OUTPUT_CUDA_RGBA_UINT8, m_leftFrame));
            CHECK_DW_ERROR(dwSensorCamera_getImage(&rightRGBA, DW_CAMERA_OUTPUT_CUDA_RGBA_UINT8, m_rightFrame));

            gStereoImages[0] = leftRGBA;
            gStereoImages[1] = rightRGBA;
...
int main(int argc, const char** argv)
{
    ProgramArguments args(argc, argv, {
                                         ...
                                         ProgramArguments::Option_t{"live", "0", "If set to 1, use live GMSL cameras from rig"},
                                      });
