    ...
    dwVehicleIOActuationFeedback* actuationFeedback)
{
    ...
    *safetyState = m_vehicleState->stateBuffer.pendingSafety;
    ...
    // Mark steeringWheelAngle as valid if we have steering
    if (m_vehicleState->stateBuffer.hasSteering && safetyState->timestamp_us > 0) {
        dwSignal_encodeSignalValidity(
            &safetyState->validityInfo.steeringWheelAngle,
            DW_SIGNAL_STATUS_LAST_VALID,
            DW_SIGNAL_TIMEOUT_NONE,
    ...
    *nonSafetyState = m_vehicleState->stateBuffer.pendingNonSafety;
    ...
    if (m_vehicleState->stateBuffer.hasSpeed && m_vehicleState->stateBuffer.lastSpeedUpdate > 0) {
        dwSignal_encodeSignalValidity(
            &nonSafetyState->validityInfo.speedESC,
            DW_SIGNAL_STATUS_LAST_VALID,
            DW_SIGNAL_TIMEOUT_NONE,
    ...
    // Mark frontSteeringAngle valid if we have steering
    if (m_vehicleState->stateBuffer.hasSteering && m_vehicleState->stateBuffer.lastSteeringUpdate > 0) {
        dwSignal_encodeSignalValidity(
            &nonSafetyState->validityInfo.frontSteeringAngle,
            DW_SIGNAL_STATUS_LAST_VALID,
            DW_SIGNAL_TIMEOUT_NONE,
    ...
    // Mark wheelSpeed/ wheelTicks valid for all wheels if we have wheel speeds
    if (m_vehicleState->stateBuffer.hasWheelSpeeds && m_vehicleState->stateBuffer.lastWheelSpeedUpdate > 0) {
        for (int i = 0; i < 4; ++i) {
            dwSignal_encodeSignalValidity(
                &nonSafetyState->validityInfo.wheelSpeed[i],
                DW_SIGNAL_STATUS_LAST_VALID,
