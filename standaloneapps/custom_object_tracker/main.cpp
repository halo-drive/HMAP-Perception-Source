...
public:
    ...
    bool onInitialize() override
    {
        ...
        //------------------------------------------------------------------------------
        {
            ...
            {
                // Prefer rig-based initialization if provided
                const std::string rigFile = getArgument("rig");
                if (!rigFile.empty())
                {
                    CHECK_DW_ERROR(dwRig_initializeFromFile(&m_rigConfig, m_sdk, rigFile.c_str()));

                    uint32_t cameraCount = 0;
                    CHECK_DW_ERROR(dwRig_getSensorCountOfType(&cameraCount, DW_SENSOR_CAMERA, m_rigConfig));
                    if (cameraCount == 0)
                    {
                        std::cerr << "Rig has no camera sensors: " << rigFile << std::endl;
                        return false;
                    }
                    ...
                    const char* rigParams = nullptr;
                    CHECK_DW_ERROR(dwRig_getSensorProtocol(&protocol, cameraSensorIdx, m_rigConfig));
                    CHECK_DW_ERROR(dwRig_getSensorParameterUpdatedPath(&rigParams, cameraSensorIdx, m_rigConfig));

                    params.protocol   = protocol;
                    params.parameters = rigParams;
    ...
    ProgramArguments args(argc, argv,
                          {
                              ...
                              ProgramArguments::Option_t("rig", "", "path to rig JSON for live multi-camera initialization"),
