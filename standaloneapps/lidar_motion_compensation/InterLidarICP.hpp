...
// Include the SygnalPomoParser for CAN message processing
...
private:
    ...
    std::string m_tensorRTEngine;  // Path to TensorRT engine file
    uint32_t m_maxIters;
    ...
    // Egomotion and Vehicle IO Handles
    ...
    // Vehicle state tracking for egomotion
    ...
    // SygnalPomoParser for CAN message processing
    ...
    int m_maxPoints;      // Maximum number of points the model expects
    int m_numFeatures;    // Number of features per point (usually 4 for x,y,z,intensity)
    uint32_t m_requiredPoints;  // Number of points to collect before inference
    uint32_t m_maxModelPoints;  // Maximum points the model can handle
    
    // Model-specific constants (from working implementation)
    ...
    // Sensor Information
    // ------------------------------------------------
    uint32_t m_lidarCount = 0; // LiDAR count to use automatically set accumulation and stitching number in the loop
    
    // Sensor indices for processing (like egomotion sample)
    ...
    WindowTile m_trajectoryTile;          // Trajectory visualization view
    ...
    uint32_t m_trajectoryRenderBufferId = 0;
    std::vector<dwTransformation3f> m_trajectoryHistory;
    static constexpr uint32_t MAX_TRAJECTORY_POINTS = 1000;
    ...
    // Egomotion and CAN Parser initialization (no VehicleIO needed - uses SygnalPomoParser directly)
    void initEgomotion();
    void initCANParser();
    ...
    // Egomotion and trajectory processing
    void processCANMessages();
    void updateTrajectory();
    void updateRigToWorldTransform();
    ...
    void initTrajectoryRenderBuffer();
