    ...
    , m_stream(cudaStreamDefault)
{
    ...
    if (m_objectDetectionEnabled) {
        std::cout << "    TensorRT Engine: " << m_tensorRTEngine << std::endl;
    }
...
void InterLidarICP::initSensors()
{
    ...
    // Get LiDAR count (required to set automatically the accumulation and stitching number in the loop)
    ...
    // Discover and log sensor names and store indices
    std::cout << "Discovering sensors in rig:" << std::endl;
    uint32_t sensorCount = 0;
    CHECK_DW_ERROR(dwRig_getSensorCount(&sensorCount, m_rigConfig));
    
    for (uint32_t i = 0; i < sensorCount; i++) {
        dwSensorType type;
        const char* name;
        CHECK_DW_ERROR(dwRig_getSensorType(&type, i, m_rigConfig));
        CHECK_DW_ERROR(dwRig_getSensorName(&name, i, m_rigConfig));
        
        if (type == DW_SENSOR_IMU || type == DW_SENSOR_CAN || type == DW_SENSOR_DATA || type == DW_SENSOR_GPS) {
            const char* typeName = (type == DW_SENSOR_IMU) ? "IMU" : 
                                 (type == DW_SENSOR_CAN || type == DW_SENSOR_DATA) ? "CAN" : "GPS";
            std::cout << "  " << typeName << ": " << name << " (index: " << i << ")" << std::endl;
            
            // Store sensor indices for processing
            if (type == DW_SENSOR_IMU && m_imuSensorIdx == 0) {
                m_imuSensorIdx = i;
            }
            if ((type == DW_SENSOR_CAN || type == DW_SENSOR_DATA) && m_vehicleSensorIdx == 0) {
                m_vehicleSensorIdx = i;
            }
            if (type == DW_SENSOR_GPS && m_gpsSensorIdx == 0) {
                m_gpsSensorIdx = i;
            }
        }
    }
    
    // Log sensor configuration status
    if (m_imuSensorIdx > 0 && m_vehicleSensorIdx > 0) {
        std::cout << "✓ Motion compensation and egomotion will be enabled (IMU + CAN)" << std::endl;
    } else if (m_imuSensorIdx > 0) {
        std::cout << "✓ Motion compensation will be enabled (IMU only)" << std::endl;
    } else {
        std::cout << "⚠ Motion compensation will be limited - no IMU sensor found" << std::endl;
    }
...
void InterLidarICP::initAccumulation()
{
    for (uint32_t i = 0; i < m_lidarCount; i++) {
        ...
        params.enableMotionCompensation = true;  // Enable motion compensation with egomotion
...
void InterLidarICP::initRendering()
{
    ...
    // Initialize trajectory render buffer
    initTrajectoryRenderBuffer();
...
bool InterLidarICP::initializeObjectDetection() {
    ...
    std::cout << "Initializing object detection..." << std::endl;
    
    // Calculate input requirements
    calculateInputRequirements();
    ...
    // Point cloud buffer allocation removed - was unused
    
    std::cout << "Object detection initialized successfully" << std::endl;
    return true;
}
...
// initVehicle() method removed - not needed for egomotion
// The egomotion sample uses SygnalPomoParser directly instead of VehicleIO

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::initEgomotion()
{
    // Initialize Egomotion parameters - use actual sensor names from rig
    dwEgomotionParameters egomotionParams{};
    
    // Find sensor names from rig configuration
    std::string imuSensorName = "";
    std::string canSensorName = "";
    ...
    for (uint32_t i = 0; i < sensorCount; i++) {
        ...
        if (type == DW_SENSOR_IMU && imuSensorName.empty()) {
            imuSensorName = name;
            std::cout << "Found IMU sensor: " << name << std::endl;
        }
        if ((type == DW_SENSOR_CAN || type == DW_SENSOR_DATA) && canSensorName.empty()) {
            canSensorName = name;
            std::cout << "Found CAN sensor: " << name << std::endl;
        }
    }
    
    // Initialize egomotion with found sensor names (CAN is optional)
    const char* canName = canSensorName.empty() ? nullptr : canSensorName.c_str();
    CHECK_DW_ERROR_MSG(dwEgomotion_initParamsFromRig(&egomotionParams, m_rigConfig, imuSensorName.c_str(), canName),
                       "Could not get egomotion parameters from rig");
    
    // Configure egomotion parameters
    ...
    egomotionParams.historySize = 1000; // Default history size
    
    CHECK_DW_ERROR_MSG(dwEgomotion_initialize(&m_egomotion, &egomotionParams, m_context),
                       "Could not initialize Egomotion");
    
    std::cout << "Egomotion initialized successfully" << std::endl;
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::initCANParser()
{
    if (m_vehicleSensorIdx == 0) {
        std::cout << "No CAN sensors found - CAN parser will not be initialized" << std::endl;
        return;
    }
    
    // Initialize the CAN parser
    m_canParser = std::unique_ptr<SygnalPomoParser>(new SygnalPomoParser());
    
    // Load vehicle configuration (same approach as egomotion sample)
    SygnalPomoParser::VehicleCANConfiguration config;
    if (!m_canParser->loadVehicleConfiguration(config)) {
        std::cerr << "Failed to load vehicle configuration for CAN parser" << std::endl;
        throw std::runtime_error("CAN parser configuration failed");
    }
    
    // Configure speed measurement type
    m_canParser->configureSpeedMeasurementType(DW_EGOMOTION_REAR_SPEED);
    
    std::cout << "CAN parser initialized successfully with vehicle configuration" << std::endl;
    std::cout << "CAN-ID Config: Speed=0x" << std::hex << config.speedCANId 
              << ", Steering=0x" << config.steeringWheelAngleCANId 
              << ", Wheels=0x" << config.wheelSpeedCANId << std::dec << std::endl;
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::processCANMessages()
{
    // This method is called from the main processing loop
    // CAN messages are already processed in getSpinFromBothLidars()
    // This is a placeholder for any additional CAN processing if needed
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::updateTrajectory()
{
    // Update trajectory history with current rig-to-world transformation
    m_trajectoryHistory.push_back(m_currentRigToWorld);
    
    // Limit trajectory history size
    if (m_trajectoryHistory.size() > MAX_TRAJECTORY_POINTS) {
        m_trajectoryHistory.erase(m_trajectoryHistory.begin());
    }
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::updateRigToWorldTransform()
{
    // Update the rig-to-world transformation using egomotion
    // This is similar to the PointCloudProcessingSample approach
    if (m_egomotion != DW_NULL_HANDLE) {
        // Get the current transformation from egomotion
        dwTransformation3f egomotionTransform = DW_IDENTITY_TRANSFORMATION3F;
        
        // Use egomotion to compute the transformation
        // For now, we'll use the cumulative ICP transform as the base
        // In a full implementation, you would integrate egomotion data here
        m_currentRigToWorld = m_cumulativeICPTransform;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::initTrajectoryRenderBuffer()
{
    // Initialize trajectory rendering buffer
    CHECK_DW_ERROR(dwRenderEngine_createBuffer(&m_trajectoryRenderBufferId,
    ...
    std::cout << "Trajectory render buffer initialized" << std::endl;
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::renderTrajectory()
{
    if (m_trajectoryHistory.size() < 2) {
        return; // Need at least 2 points to draw a line
    }
    
    // Set trajectory tile
    dwRenderEngine_setTile(m_trajectoryTile.tileId, m_renderEngine);
    
    // Set up rendering parameters
    dwRenderEngine_setColor({1.0f, 1.0f, 0.0f, 1.0f}, m_renderEngine); // Yellow trajectory
    dwRenderEngine_setLineWidth(3.0f, m_renderEngine);
    
    // Render trajectory line strip
    dwRenderEngine_render(DW_RENDER_ENGINE_PRIMITIVE_TYPE_LINESTRIP_3D,
                          m_trajectoryHistory.data(),
                          sizeof(dwTransformation3f),
                          offsetof(dwTransformation3f, array) + 3 * 4 * sizeof(float32_t), // Extract translation part
                          static_cast<uint32_t>(m_trajectoryHistory.size()),
                          m_renderEngine);
    
    // Render coordinate frame at current position
    if (!m_trajectoryHistory.empty()) {
        renderCoordinate(m_trajectoryHistory.back());
    }
}

/////////////////////////////////////////////////////////////////////////////////////////////////
void InterLidarICP::renderCoordinate(const dwTransformation3f& rigToWorld)
{
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        // Axis
        // Simple matrix multiplication for axis direction
        arrow[1].x = rigToWorld.array[0] * localAxis[0] + rigToWorld.array[4] * localAxis[1] + rigToWorld.array[8] * localAxis[2] + rigToWorld.array[12];
        arrow[1].y = rigToWorld.array[1] * localAxis[0] + rigToWorld.array[5] * localAxis[1] + rigToWorld.array[9] * localAxis[2] + rigToWorld.array[13];
        arrow[1].z = rigToWorld.array[2] * localAxis[0] + rigToWorld.array[6] * localAxis[1] + rigToWorld.array[10] * localAxis[2] + rigToWorld.array[14];
...
bool InterLidarICP::onInitialize()
{
    try {
        initDriveWorks();
        initSensors();        // Initialize rig and sensors FIRST (needed for egomotion)
        initEgomotion();      // Then initialize egomotion (needs m_rigConfig)
        initCANParser();      // Then initialize CAN parser
        initBuffers();
        initAccumulation();
        initICP();
        initStitching();
...
bool InterLidarICP::getSpinFromBothLidars()
{
    ...
    while (numLidarsAccumulated < m_lidarCount) {
        ...
        case DW_SENSOR_CAN:
        {
            if (m_vehicleSensorIdx > 0 && acquiredEvent->sensorIndex == m_vehicleSensorIdx) {
                // Process CAN messages using SygnalPomoParser (like egomotion sample)
                if (m_canParser && m_canParser->isInitialized()) {
                    m_canParser->processCANFrame(acquiredEvent->canFrame);
                }
                
                // Process CAN data through SygnalPomoParser for egomotion (like egomotion sample)
                if (m_canParser && m_canParser->isInitialized()) {
                    dwVehicleIOSafetyState safetyState;
                    dwVehicleIONonSafetyState nonSafetyState;
                    
                    // Get temporally synchronized state from CAN parser
                    if (m_canParser->getTemporallySynchronizedState(&safetyState, &nonSafetyState)) {
                        // Validate speed range (like egomotion sample)
        ...
        case DW_SENSOR_IMU:
        {
            if (m_imuSensorIdx > 0 && acquiredEvent->sensorIndex == m_imuSensorIdx) {
                dwIMUFrame imuFrame = acquiredEvent->imuFrame;
        ...
        case DW_SENSOR_GPS:
        {
            if (m_gpsSensorIdx > 0 && acquiredEvent->sensorIndex == m_gpsSensorIdx) {
                // Process GPS frame for egomotion
                dwGPSFrame gpsFrame = acquiredEvent->gpsFrame;
                // Note: GPS processing for egomotion would go here if needed
                // For now, we'll just log that GPS data is available
                if (m_verbose) {
                    std::cout << "GPS frame received at timestamp: " << gpsFrame.timestamp_us << std::endl;
                }
            }
            break;
        }
...
void InterLidarICP::stitchPointClouds()
{
    ...
    // Enable motion compensation for stitching using egomotion
    dwTime_t registrationTime = m_registrationTime[LIDAR_A_INDEX]; // Use LiDAR A as reference
    CHECK_DW_ERROR_MSG(dwPointCloudStitcher_enableMotionCompensation(registrationTime, m_egomotion, m_stitcher),
                       "Could not enable motion compensation for stitching");
...
void InterLidarICP::performObjectDetection()
{
    ...
    // Step 1: Run inference (now includes point cloud preparation)
    performInference();
    ...
    if (m_verbose) {
        std::cout << "Object detection completed in " << duration.count() << " ms" << std::endl;
        std::cout << "Detected " << m_currentBoxes.size() << " objects" << std::endl;
    }
}
...
void InterLidarICP::onProcess()
{
    ...
    // Step 4.5: Update trajectory and rig-to-world transform
    updateRigToWorldTransform();
...
void InterLidarICP::onRender()
{
    ...
    // Render trajectory if we have trajectory data
    if (!m_trajectoryHistory.empty()) {
        dwRenderEngine_setTile(m_icpTile.tileId, m_renderEngine);
        
        // Set proper 3D rendering state for trajectory
        dwMatrix4f modelView;
        Mat4_AxB(modelView.array, getMouseView().getModelView()->array, DW_IDENTITY_TRANSFORMATION3F.array);
        dwRenderEngine_setModelView(&modelView, m_renderEngine);
        dwRenderEngine_setProjection(getMouseView().getProjection(), m_renderEngine);
        ...
        if (m_verbose) {
            std::cout << "[RENDER-DEBUG] Trajectory rendered with " << m_trajectoryHistory.size() << " points" << std::endl;
        }
    }
...
void InterLidarICP::onRelease()
{
    ...
    // Release trajectory render buffer
    if (m_trajectoryRenderBufferId != 0) {
        dwRenderEngine_destroyBuffer(m_trajectoryRenderBufferId, m_renderEngine);
    }
    ...
    // Release egomotion and vehicle IO
    if (m_egomotion) {
