...
public:
    ...
    // Public accessors for IPC producer
public:
    dwContextHandle_t getContext() { return m_context; }
    uint32_t getFrameNum() { return m_frameNum; }
    AlignmentState getAlignmentState() { return m_alignmentState; }
    const dwPointCloud& getStitchedPointsHost() { return m_stitchedPointsHost; }
    const std::vector<BoundingBox>& getCurrentBoxes() { return m_currentBoxes; }
    const dwPointCloudExtractedPlane& getFilteredGroundPlane() { return m_filteredGroundPlane; }
    bool isFilteredGroundPlaneValid() { return m_filteredGroundPlaneValid; }
    const std::vector<float32_t>& getFreeSpacePoints() { return m_freeSpacePoints; }
    bool isFreeSpaceEnabled() { return m_freeSpaceEnabled; }
