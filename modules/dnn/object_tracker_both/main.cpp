...
private:
    ...
    dwSensorHandle_t m_cameraHandle = DW_NULL_HANDLE;
...
public:
    ...
    bool onInitialize() override
    {
        ...
        //------------------------------------------------------------------------------
        // Store input type
        m_inputType = getArgument("input-type");

        // Initialize Camera based on input type
        if (m_inputType == "usb") {
            ...
            CHECK_DW_ERROR(dwSAL_createSensor(&m_cameraHandle, params, m_sal));

            // Log available modes
            uint32_t numModes = 0;
            CHECK_DW_ERROR(dwSensorCamera_getNumSupportedCaptureModes(&numModes, m_cameraHandle));

            if (numModes > 1) {
                dwCameraProperties properties{};
                CHECK_DW_ERROR(dwSensorCamera_getSensorProperties(&properties, m_cameraHandle));

                for (uint32_t modeIdx = 0; modeIdx < numModes; ++modeIdx) {
                    dwCameraProperties mode{};
                    CHECK_DW_ERROR(dwSensorCamera_getSupportedCaptureMode(&mode, modeIdx, m_cameraHandle));
                    std::cout << "Mode " << modeIdx << ": " << mode.resolution.x << "x" << mode.resolution.y << " " 
                            << mode.framerate << " fps" << std::endl;
                }
            }

            // Retrieve camera dimensions
            dwImageProperties cameraImageProps{};
            CHECK_DW_ERROR(dwSensorCamera_getImageProperties(&cameraImageProps, DW_CAMERA_OUTPUT_NATIVE_PROCESSED, m_cameraHandle));
            
            m_cameraImageType = cameraImageProps.type;
            m_imageWidth = cameraImageProps.width;
            m_imageHeight = cameraImageProps.height;
            ...
            // Create RGBA image
            dwImageProperties rgbaProperties = cameraImageProps;
            rgbaProperties.format = DW_IMAGE_FORMAT_RGBA_UINT8;
            rgbaProperties.type = DW_IMAGE_CUDA;
            CHECK_DW_ERROR(dwImage_create(&m_imageRGBA, rgbaProperties, m_sdk));

            // Create image streamer
            CHECK_DW_ERROR(dwImageStreamerGL_initialize(&m_streamerGL, &rgbaProperties, DW_IMAGE_GL, m_sdk));
            
            // Start camera
            CHECK_DW_ERROR(dwSensor_start(m_cameraHandle));
    }else {
            dwSensorParams params{};
        ...
        //------------------------------------------------------------------------------
        {
            ...
            // Detection region
            float expansionFactor = 2.6f;
            m_detectionRegion.width = std::min(static_cast<uint32_t>(m_networkInputDimensions.width * expansionFactor),
                                               m_imageWidth);
            m_detectionRegion.height = std::min(static_cast<uint32_t>(m_networkInputDimensions.height * expansionFactor),
                                                m_imageHeight);
    ...
    // Return GL resources based on camera type
    if (m_inputType == "usb") {
        // For USB camera, return GL resources via dwImageStreamerGL
        dwImageHandle_t imageGL = (dwImageHandle_t)m_imgGl;
        CHECK_DW_ERROR(dwImageStreamerGL_consumerReturn(&imageGL, m_streamerGL));
    ...
    if (m_inputType == "usb") {
        if (m_frame) {
            ...
            m_frame = nullptr; // Prevent double return
        }
    } else {
        if (m_frame) {
            CHECK_DW_ERROR(dwSensorCamera_returnFrame(&m_frame));
    ...
    void onRelease() override
{
    ...
    if (m_inputType == "usb")
    {
        ...
        CHECK_DW_ERROR(dwSensor_stop(m_cameraHandle));
        if (m_streamerGL != DW_NULL_HANDLE)
        {
            CHECK_DW_ERROR(dwImageStreamerGL_release(m_streamerGL));
        }
        CHECK_DW_ERROR(dwSAL_releaseSensor(m_cameraHandle));
    }
    else
    {
        // Clean up GMSL/virtual camera resources
        m_camera.reset();
        if (m_streamerCUDA2GL)
        {
...
private:
    ...
    void getNextFrame(dwImageCUDA** nextFrameCUDA, dwImageGL** nextFrameGL) {
        if (m_inputType == "usb") {
            // USB camera path
            dwStatus result = dwSensorCamera_readFrame(&m_frame, 50000, m_cameraHandle);
            if (result != DW_SUCCESS) {
                if (result == DW_TIME_OUT) {
                    return;
                } else if (result == DW_NOT_AVAILABLE) {
                    std::cerr << "Camera is not running or not found" << std::endl;
                    stop();
                    return;
                } else {
                    std::cerr << "Cannot get frame from camera: " << dwGetStatusName(result) << std::endl;
                    stop();
                    return;
                }
            }
            ...
            // Convert if needed and get CUDA image
            if (m_cameraImageType != DW_IMAGE_CUDA || outputType != DW_CAMERA_OUTPUT_CUDA_RGBA_UINT8) {
                CHECK_DW_ERROR(dwImage_copyConvert(m_imageRGBA, m_cameraImage, m_sdk));
                CHECK_DW_ERROR(dwImage_getCUDA(nextFrameCUDA, m_imageRGBA));
            } else {
                CHECK_DW_ERROR(dwImage_getCUDA(nextFrameCUDA, m_cameraImage));
            }
            
            // Stream to GL for rendering - use only dwImageStreamerGL for USB
            dwImageHandle_t frameGL = nullptr;
            CHECK_DW_ERROR(dwImageStreamerGL_producerSend(m_cameraImage, m_streamerGL));
            CHECK_DW_ERROR(dwImageStreamerGL_consumerReceive(&frameGL, 30000, m_streamerGL));
            *nextFrameGL = (dwImageGL*)frameGL;
        } else {
            // GMSL/virtual camera path
    ...
    const std::vector<std::string> m_targetClasses = {
    ...
    void interpretOutput(const float32_t* outConf, const dwRect* const roi)
    {
        ...
        for (uint32_t i = 0; i < tmpResAfterNMS.size(); i++)
        {
            ...
            for (const auto& targetClass : m_targetClasses) {
    ...
    ProgramArguments args(argc, argv,
                          {
                              ...
                              ProgramArguments::Option_t("input-type", "camera", "input type: camera (for gmsl), usb, or virtual"),
                              ProgramArguments::Option_t("device", "0", "USB device id (for usb camera)"),
                              ProgramArguments::Option_t("mode", "", "USB camera mode (optional)"),
